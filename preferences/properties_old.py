...
class ResizeAssistProperties(bpy.types.PropertyGroup):
    ...
    def update_multi(self, context):
        x = bpy.context.scene.resize_pro.dir_x
        y = bpy.context.scene.resize_pro.dir_y
        z = bpy.context.scene.resize_pro.dir_z
        if not bpy.context.scene.resize_pro.multi:
            if [x, y, z].count(True) == 2:
                condition = self.condition
                latest = condition["latest"]
                def enable_x(self, context):
                    print("enableX")
                    bpy.context.scene.resize_pro.dir_x = True
                    bpy.context.scene.resize_pro.dir_y = False
                    bpy.context.scene.resize_pro.dir_z = False
                    condition["oldest"] = "x"
                    condition["latest"] = "x"
                def enable_y(self, context):
                    print("enableY")
                    bpy.context.scene.resize_pro.dir_x = False
                    bpy.context.scene.resize_pro.dir_y = True
                    bpy.context.scene.resize_pro.dir_z = False
                    condition["oldest"] = "y"
                    condition["latest"] = "y"
                def enable_z(self, context):
                    ...
                    bpy.context.scene.resize_pro.dir_x = False
                    bpy.context.scene.resize_pro.dir_y = False
                    bpy.context.scene.resize_pro.dir_z = True
                    condition["oldest"] = "z"
                    condition["latest"] = "z"
                if latest == "x": enable_x(self, context)
                if latest == "y": enable_y(self, context)
        ...
        if not busy:
            ...
            if count == 1:  # 軸が一つだけ選択されているとき
                ...
                if z == True:
                    ...
                    condition["latest"] = "z"
            if count >= 2:
                if bpy.context.scene.resize_pro.multi:
                    if count == 2:  # 軸が二つ選択されているとき
                        if x and not oldest == "x":  # X軸が選択されていてoldestでないなら(=新しく選択された)
                            condition["latest"] = "x"  # latestをxに設定(以下同じ)
                        if y and not oldest == "y":
                        ...
                        if oldest == "x":  # oldestがxなら(=最初に選択されたのがX軸なら)
                            bpy.context.scene.resize_pro.dir_x = False  # X軸のチェックを解除する(以下同じ)
                            if latest == "y":  # latestがyなら(2つ目に選択されたのがyなら)
                                condition["latest"] = "z"  # zをlatestに設定
                                condition["oldest"] = "y"  # yをoldestに設定(以下同じ)
                            elif latest == "z":
